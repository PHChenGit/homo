...
#             nn.Linear(128, 64),
#             nn.ReLU(),
#             nn.Linear(64, 1),  # pred heading angle (-180, 180]
#         )
#
#     def forward(self, x):
#         x = self.model(x)
...
class GeoCLIP(nn.Module):
    ...
    def predict(self, image, top_k):
        ...
        top_pred_img = self.img_gallery[top_pred.indices[0]]
        # pred_img = transforms.functional.to_pil_image(top_pred_img[0]) # pred
        top_pred_prob = top_pred.values[0]

        return top_pred_gps, top_pred_prob, top_pred_img
